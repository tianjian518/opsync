...
3. 测试通过后，该账号子页面出现「搬运路线」区：点 **+ 添加路线**，每条点「浏览选择」在目录树里点出**源目录**和**目标目录**（不用手敲路径），再选**搬运文件大小范围**（见下）、模式与调度、是否启用。
...
- 路线标题旁出现 **⏳ 运行中**，该条的「运行」按钮变灰为「运行中…」并禁用
- 同时**出现「终止」按钮**（平时隐藏）

点「终止」即可停住这条路线；也可用账号级的 **「终止本账号全部」** 一次停掉该账号下所有在跑的路线。
